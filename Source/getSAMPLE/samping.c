...
SAMPLING_ADC adcNode[2] =
		{ { .g_uiSamplingFreq = 8000, .g_ucDataReady = 0, .g_ulBadPeriphIsr1 =
				0, .g_ulBadPeriphIsr2 = 0, .g_uluDMAErrCount = 0 } };
//------------------------------------------------------------------------
...
// If the channel's not done capturing, we have an error
	if (uDMAChannelIsEnabled(UDMA_CHANNEL_ADC3)) {
		...
		ADCIntDisable(ADC0_BASE, SEQUENCER3);
...
// The amount of data transferred increments in sets of 1024
	...
	if (NUM_SAMPLES > ulDataXferd) {
		...
		if (adcNode[0].g_ucDataReady) {
			memcpy(output2, g_ulADCValues, 1024);
			countFullData++;
		}
	}
}
...
// Should go from 0 to 2048
	...
	ADCSequenceConfigure(ADC0_BASE, SEQUENCER3, ADC_TRIGGER_TIMER, 0);
	ADCSequenceStepConfigure(ADC0_BASE, SEQUENCER3, 0, ADC_CTL_CH0 |
