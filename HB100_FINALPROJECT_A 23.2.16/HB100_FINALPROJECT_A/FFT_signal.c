...
void FFTprocessing1(uint32_t *inFFT, float32_t* outFFT) {
	...
	ComplexBufFFT(inFFT, complexBuffer, FFT_LENGTH);
	...
	arm_cmplx_mag_f32(complexBuffer, outFFT, FFT_LENGTH);
	...
	arm_max_f32(outFFT, FFT_LENGTH / 2, &fftNode[0].maxValue,
			&fftNode[0].maxIndex);
	arm_mean_f32(outFFT, FFT_LENGTH, &fftNode[0].averageValue);
...
//	arm_rfft_f32(&fftStructure, outFFT, outFFT);
	...
	arm_max_f32(outFFT, NUM_SAMPLES, &fftNode[0].maxValue, &m);
	// calculate frequency value of peak bin
	fftNode[0].hertz = fftNode[0].HerztPerBin * (float32_t) m;
